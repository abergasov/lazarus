<script lang="ts">
  ...
  async function handleDocUpload(file: File) {
    if (!visit) return;
    await documents.upload(file, visit.id);
  }
    ...
    <div class="phase-content">
      {#if phaseTab === 0}
        ...
        <div class="section">
          ...
          {#if visit.plan?.priorities && visit.plan.priorities.length > 0}
            <div class="card">
              <h3>Questions to Ask</h3>
              {#each visit.plan.priorities as p}
                <div class="question-row">
                  <span class="q-text">{p.concern}</span>
                  <span class="q-urgency" class:high={p.urgency === 'high'}>{p.urgency}</span>
                </div>
              {/each}
            </div>
          {:else}
            <p class="hint">AI will generate questions once you upload documents or start preparing.</p>
          {/if}
        </div>
      ...
      {:else}
        ...
        <div class="section">
          ...
          {#if visit.plan?.outcomes?.summary}
            <div class="card">
              <p>{visit.plan.outcomes.summary}</p>
            </div>
          {:else}
            <p class="hint">AI will generate a summary after your visit is processed.</p>
          {/if}
        </div>

        {#if visit.plan?.action_items && visit.plan.action_items.length > 0}
          <div class="section">
            <h2>Action Items</h2>
            <div class="card">
              {#each visit.plan.action_items as item}
...
<style>
  ...
  .ask-float {
    ...
    box-shadow: 0 4px 16px rgba(0,122,255,0.3); transition: transform 0.15s; z-index: 50;
  }
