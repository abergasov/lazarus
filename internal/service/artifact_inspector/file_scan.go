...
func (s *Service) renderPDFPages(ctx context.Context, artifact *entities.Artifact, pdfPath string) error {
	...
	cmd := exec.CommandContext(ctx, "pdftoppm", "-png", "-r", "150", pdfPath, prefix) //nolint:gosec // variables is result of os.CreateTemp, no user input
...
func (s *Service) uploadPDFPageImage(ctx context.Context, artifact *entities.Artifact, pageNum int, filePath string) error {
	f, err := os.Open(filePath) //nolint:gosec // filePath is generated by us, no user input
	if err != nil {
	...
	if err = s.repo.CreateArtifactDerivative(ctx, &entities.ArtifactDerivatives{
		...
		PageNum:      sql.NullInt32{Int32: int32(pageNum), Valid: true}, //nolint:gosec // no overflow
