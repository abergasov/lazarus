...
func NewService(
	...
	if cfg.MaxUploadSizeBytes <= 0 {
		log.Fatal("wr", fmt.Errorf("max upload size must be positive"))
	}
...
func (s *Service) GetArtifactByID(ctx context.Context, id uuid.UUID, userID uuid.UUID) (*entities.Artifact, error) {
	return s.repo.GetArtifactByID(ctx, id, userID)
}

func (s *Service) ListArtefactsByUser(ctx context.Context, userID uuid.UUID) ([]*entities.Artifact, error) {
	return s.repo.GetAllArtifactsByOwner(ctx, userID)
}

func (s *Service) DeleteArtifact(ctx context.Context, id uuid.UUID, userID uuid.UUID) error {
	return s.repo.DeleteArtifact(ctx, id, userID)
}
